...
export default function PlayerPage() {
  ...
  return (
    <div className="flex h-screen flex-col bg-[#1e1e1e]">
      {/* ─── Slim header ─── */}
      <header className="flex h-10 shrink-0 items-center justify-between border-b border-gray-800 bg-[#252526] px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={videoPanelVisible ? "Hide video (V)" : "Show video (V)"}
          >
          ...
          {isCheckpointActive ? null : playback.isInteractive ? (
            <button
              ...
              className="inline-flex items-center gap-1 rounded border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[10px] font-medium text-amber-300 transition-colors hover:bg-amber-500/20"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1 rounded border border-gray-700 bg-gray-800 px-2 py-0.5 text-[10px] font-medium text-gray-300 transition-colors hover:bg-gray-700 hover:text-white"
            >
              <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
      ...
      {/* ─── Main content: vertical split (editor top, console bottom) ─── */}
      <PanelGroup direction="vertical" className="flex-1 min-h-0">
        {/* TOP: File explorer + Editor + floating video */}
        <Panel defaultSize={75} minSize={40} id="editor-area">
          <div className="flex h-full relative">
            {/* Checkpoint panel (full-width overlay when active) */}
            ...
            {playback.isInteractive && !isCheckpointActive && (
              <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between border-b border-amber-500/20 bg-amber-500/5 px-4 py-1">
                <span className="text-[10px] text-amber-300">
                  Interactive mode — edit freely
                </span>
                <button
                  type="button"
                  onClick={playback.exitInteractive}
            ...
            <button
              ...
              className="flex w-4 shrink-0 items-center justify-center border-r border-gray-800 bg-[#1e1e1e] text-gray-600 transition-colors hover:bg-gray-800 hover:text-gray-400"
            ...
            {/* ─── Floating video overlay (PiP style, bottom-right) ─── */}
            {videoPanelVisible && (
              <div className="absolute bottom-3 right-3 z-10 w-52 overflow-hidden rounded-lg shadow-2xl ring-1 ring-gray-700/50">
                {playback.videoUrl ? (
                  <video
                  ...
                  <div className="flex aspect-video items-center justify-center bg-gray-900">
                    <svg className="h-6 w-6 text-gray-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" />
                    </svg>
                  </div>
                )}
        ...
        {/* BOTTOM: Console / Output / Preview (resizable) */}
        <Panel defaultSize={25} minSize={8} maxSize={60} id="console">
          <div className="flex h-full flex-col border-t border-gray-800">
            {/* Console header */}
            <div className="flex h-7 shrink-0 items-center justify-between border-b border-gray-800 bg-[#252526] px-3">
              <span className="text-[10px] font-medium uppercase tracking-wider text-gray-500">
                {activeEditorSlide ? "Slide" : useCodeRunner ? "Console" : "Preview"}
              </span>
              {activeEditorSlide && (
        ...
        >
          ...
          {playback.durationMs > 0 && playback.checkpoints.map((cp) => {
            ...
            return (
              <div
                key={cp.id}
                className="absolute top-1/2 -translate-y-1/2 h-2.5 w-1 rounded-sm bg-blue-400/80 pointer-events-none"
        ...
        <div className="flex items-center justify-between px-3 py-1.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="rounded px-1.5 py-0.5 font-mono text-[11px] text-gray-400 transition-colors hover:bg-gray-800 hover:text-white"
